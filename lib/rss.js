...
export async function generateRss(props) {
  ...
  const followChallengeXml = `
  <follow_challenge>
    <feed_id>132336729654359040</feed_id>
    <user_id>116180320394648576</user_id>
  </follow_challenge>`
  let rssXml = feed.rss2().replace('</channel>', `${followChallengeXml}</channel>`)
  let atomXml = feed.atom1().replace('</feed>', `${followChallengeXml}</feed>`)
  const jsonFeed = JSON.parse(feed.json1())
  jsonFeed.follow_challenge = {
    feed_id: 132336729654359040,
    user_id: 116180320394648576
  }

  try {
    fs.mkdirSync('./public/rss', { recursive: true })
    fs.writeFileSync('./public/rss/feed.xml', rssXml)
    fs.writeFileSync('./public/rss/atom.xml', atomXml)
    fs.writeFileSync('./public/rss/feed.json', JSON.stringify(jsonFeed, null, 2))
  } catch (error) {
